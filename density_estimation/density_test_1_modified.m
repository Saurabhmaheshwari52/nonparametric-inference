...
par_scale= 0.4*ones(1,num_dim_x_full);
par_corr = [];
par = [par_loc,par_scale,par_corr];
fun_pdf = @(x,par) mvnpdf(x,par(1:num_dim_x_full),...
    diag(par(num_dim_x_full+1:2*num_dim_x_full).^2));

% generate data points
rng(1);
obs_x_pool_full = [normrnd(par_loc(1),par_scale(1),1e6,1),normrnd(par_loc(2),...
    par_scale(2),1e6,1),normrnd(par_loc(3),par_scale(3),1e6,1)];
