...
% true density
par_loc = 10*ones(1,num_dim_x_full); 
par_scale= 0.2*ones(1,num_dim_x_full);
par_corr = [];
par = [par_loc,par_scale,par_corr];
fun_pdf = @(x,par) gampdf(x(:,1),par(1),par(4)).*gampdf(x(:,2),par(2),par(5)).*...
    gampdf(x(:,3),par(3),par(6));

% generate data points
rng(1);
obs_x_pool_full = [gamrnd(par_loc(1),par_scale(1),1e6,1),gamrnd(par_loc(2),...
    par_scale(2),1e6,1),gamrnd(par_loc(3),par_scale(3),1e6,1)];
