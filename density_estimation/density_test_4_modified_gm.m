...
% true density
par_loc = 2*ones(1,num_dim_x_full); 
...
for i =1:num_dim_x_full, obs_x_pool_full(:,i)=gaminv(obs_x_pool_full(:,i),par_loc(i),par_scale(i)); end
obs_y_pool_full = fun_map(obs_x_pool_full);

% naive integration
fun_pdf_smp = @(x) 1/(3^num_dim_x_full);

% generate numerical sampling points
rng(2);
smp_x_full = [rand(1e6,1)*3,rand(1e6,1)*3];
